...
class TrashWiew(QMainWindow, Ui_MainWindow3):
    ...
    def text_word_wrap(self):  # Преобразует текст для перевода на новую строку
        ...
        length = 0
        self.text.replace('\n', '')
        if len(self.text) > 90:  # 100 - максимальная длина строки при этом шрифте и этом размере лейбела
            text = self.text.split()
            for i in text:
                if length + len(i) < 90:
                    result += i + ' '
